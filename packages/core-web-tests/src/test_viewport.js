QUnit.module("viewport", function() {
	...
	// Polyfill not loaded in browsers that have "window.pageXOffset"
	QUnit.skip("window.scrollX", (assert) => {
		assert.ok(window.scrollX === 0, "must be 0");
	});

	// Polyfill not loaded in browsers that have "window.pageYOffset"
	QUnit.skip("window.scrollY", (assert) => {
