...
async function generateTemplate(mapping) {
	const src = await readFile(require.resolve('@webcomponents/template'), 'utf-8');
...
async function generateShadyDOM(mapping) {
	const src = await readFile(require.resolve('@webcomponents/shadydom/shadydom.min.js'), 'utf-8');
...
async function generateShadyCSS(mapping) {
	const scopingShim = await readFile(require.resolve('@webcomponents/shadycss/scoping-shim.min.js'), 'utf-8');
	await writeFile(path.join(modulesDir, '~shadycss-scoping-shim.js'), scopingShim);

	const applyShim = await readFile(require.resolve('@webcomponents/shadycss/apply-shim.min.js'), 'utf-8');
	await writeFile(path.join(modulesDir, '~shadycss-apply-shim.js'), applyShim);

	const customStyleInterface = await readFile(require.resolve('@webcomponents/shadycss/custom-style-interface.min.js'), 'utf-8');
	await writeFile(path.join(modulesDir, '~shadycss-custom-style-interface.js'), customStyleInterface);
...
async function generateCustomElements(mapping) {
	const src = await readFile(require.resolve('@webcomponents/custom-elements'), 'utf-8');
